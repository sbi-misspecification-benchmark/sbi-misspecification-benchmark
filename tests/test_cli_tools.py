...
def test_run_tool_returns_expected_output():
    ...
    assert result == 'Running the tool with evaluation: ppc, c2st'
...
def test_handle_run_command_with_valid_metrics(monkeypatch, capsys):
    ...
    assert 'Running the tool with evaluation: ppc, c2st' in captured.out
...
def test_handle_run_command_with_invalid_metrics(monkeypatch, capsys):
    ...
    handle_command(args)
    captured = capsys.readouterr()
    assert 'Invalid evaluation: nonsense' in captured.out
    assert 'Valid evaluation are:' in captured.out
